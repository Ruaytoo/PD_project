...
void
Strategy::SetStrategy (std::string & n, std::map < std::string, float >&p)
{
  name = n;
  params = p;
};
...
bool
DefectStrategy::MakeChoice (const std::vector < Agent > &agents, int
			    agent_index)
{
...
bool
CoopStrategy::MakeChoice (const std::vector < Agent > &agents,
			  int agent_index)
{
  if (agents.size () > (unsigned long) agent_index)
    return 1;
...
bool
TitForTat::MakeChoice (const std::vector < Agent > &agents, int agent_index)
{
...
void
TitForTat::SetStart (int index, bool choice)
{
  if (index == 1)
    {
...
bool
GeneticStrategy::MakeChoice (const std::vector < Agent > &agents,
			     int agent_index)
{
  ...
  for (int i = 64; i < 70; ++i)
    {
      asBinary *= 2;
      asBinary +=
	(unsigned long) agents.at ((unsigned long) agent_index).strategy->
	chromosome.at ((unsigned long) i);
    }

  return agents.at ((unsigned long) agent_index).strategy->chromosome.
    at (asBinary);
}

bool
DetectiveStrategy::MakeChoice (const std::vector < Agent > &agents,
			       int agent_index)
{
